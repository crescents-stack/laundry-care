...
const Login = () => {
  ...
  useEffect(() => {
    let data: FormDataType = JSON.parse(
      localStorage.getItem("formDataShop") as string
    );
    if (data) {
      setFormData({
        ...formData,
  ...
  const EmailVerify = async () => {
    ...
    try {
      const verifyResponse = await axios.post(
        `${process.env.BACKEND_URL}/shops/verification-latter`,
        {
  ...
  const FetchLoginAPI = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.BACKEND_URL}/shops/login`,
      ...
      if (response.status === 200) {
        ...
        if (rememberme) {
          localStorage.setItem("formDataShop", JSON.stringify(formData))
        }
